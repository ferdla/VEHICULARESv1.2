...
def agregar_modelo():
    ...
    try:
        ...
        if nueva_marca_nombre:
            ...
            commit()
            id_marca = vehiculo_model.get_last_insert_id(cur)
...
# ==============================================================================
# RESOLVER PENDIENTES (respuestas al modal del usuario)
# ==============================================================================

@bp.route('/vehiculos/resolver_regla_pendiente', methods=['POST'])
def resolver_regla_pendiente_route():
    ...
    cur = get_cursor()
    try:
        resultado = resolver_regla_pendiente(cur, id_pendiente, id_marca, confirmar, commit)
...
def resolver_pendiente_route():
    ...
    cur = get_cursor()
    try:
        resultado = resolver_excepcion_pendiente(cur, id_pendiente, id_modelo, confirmar, commit)
        return jsonify(resultado)
    except Exception as e:
        rollback()
        return jsonify({'success': False, 'error': str(e)})
