...
def get_all_empresas(cur):
    cur.execute("SELECT id_empresa, nombre_empresa, activo, fecha_creacion FROM empresa")
    return cur.fetchall()
...
def get_all_tipos_riesgo(cur):
    cur.execute("SELECT id_tipo_riesgo, nombre_riesgo, codigo_interno FROM tipo_riesgo ORDER BY id_tipo_riesgo")
    return cur.fetchall()
